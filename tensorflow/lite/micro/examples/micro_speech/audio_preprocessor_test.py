...
class AudioPreprocessorTest(test_util.TensorFlowTestCase):
  ...
  def setUp(self):
    ...
    self.int8_model_path = Path(
        self.model_prefix_path,
        'audio_preprocessor_int8.tflite')
    self.float_model_path = Path(
        self.model_prefix_path,
  ...
  def testFeatureGeneration(self):
    ...
    data: tf.Tensor = tf.random.uniform(
        minval=int(tf.dtypes.int16.min), maxval=tf.dtypes.int16.max, seed=42,
        shape=(1, window_size), dtype=tf.int32)
  ...
  def testFeatureOutputYes(self):
    ...
    expected = [
        124, 105, 126, 103, 125, 101, 123, 100, 116, 98, 115, 97, 113,
        90, 91, 82, 104, 96, 117, 97, 121, 103, 126, 101, 125, 104,
        126, 104, 125, 101, 116, 90, 81, 74, 80, 71, 83, 76, 82, 71
    ]
  ...
  def testFeatureOutputNo(self):
    ...
    feature_list = feature.numpy().tolist()
    expected = [
        126, 103, 124, 102, 124, 102, 123, 100, 118, 97, 118, 100, 118,
        98, 121, 100, 121, 98, 117, 91, 96, 74, 54, 87, 100, 87,
        109, 92, 91, 80, 64, 55, 83, 74, 74, 78, 114, 95, 101, 81
    ]
    self.assertSequenceEqual(feature_list, expected)
