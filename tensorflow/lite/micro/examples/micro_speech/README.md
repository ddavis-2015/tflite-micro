...
This model consists primarily of [Signal Library](python/tflite_micro/signal) operations.
The library is a set of Python methods, and bindings to `C++` library code.
To allow for use with the `TFLM MicroInterpreter`, a set of [Signal Library kernels](signal/micro/kernels)
is also provided.
...
The `FeatureParams` class is located within the [audio_preprocessor.py](audio_preprocessor.py)
...
The `AudioPreprocessor` class provides easy to use convenience methods for creating
...
The [audio_preprocessor.py](audio_preprocessor.py) script generates a `.tflite`
...
The [evaluate.py](evaluate.py) script predicts the category of a single audio sample
